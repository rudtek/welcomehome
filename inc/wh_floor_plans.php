<?php
function wh_floor_plans($atts) {
   $endpoint_path = 'floor_plans'; // define path here
   $html = '';
   ...
   if (is_admin() || wp_doing_ajax()) {
      ...
      if ($response_code != 200) {
         ...
         foreach ($data as $item) {
            $html .= '<li> Community ID:'.$item->community_id.' floorplan name: '.$item->name.' - '.$item->square_feet.' sf, '.$item->beds.', '.$item->baths.'baths</li>';
         }
         $html .='</ul>';
      }
   }
 ...
 add_shortcode('list-floor-plans', 'wh_floor_plans');