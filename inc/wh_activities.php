<?php
function wh_activities($atts) {
   $endpoint_path = 'activities'; // define path here
   $html = '';
   ...
   if (is_admin() || wp_doing_ajax()) {
      ...
      if ($response_code != 200) {
         ...
         foreach ($data as $item) {
            $html .= '<li>'.$item->notes.' for community '.$item->community_id.' completed by '.$item->sales_counselor_id.'</li>';
         }
         $html .='</ul>';
      }
   }
 ...
 add_shortcode('list-activities', 'wh_activities');